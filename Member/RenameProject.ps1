...
Get-ChildItem -recurse -exclude "*RenameProject.ps1*" | Select-String -pattern $TemplatePattern.ToLower() | group path | ForEach {(Get-Content $_.name).replace($TemplatePattern, $NewProjectName).replace($TemplatePatternLowCase,$NewProjectName.ToLower()) | Set-Content $_.name}
